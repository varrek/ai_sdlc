...
describe("instructions emit", () => {
  ...
  it("kiro emits AGENTS.md as a passthrough of the constitution", () => {
    const model = makeModel();
  ...
  it("emits accepted hierarchy scopes in each host's native instruction surface", () => {
    ...
    expect(kiro.get("src/core/AGENTS.md")).toContain("Role: Source module.");
    const steering = kiro.get(".kiro/steering/src-core.md")!;
    expect(steering).toContain("inclusion: fileMatch");
    expect(steering).toContain("fileMatchPattern: src/core/**");
    expect(steering).toContain("Follow `src/core/AGENTS.md`");
    expect(steering).toContain("Generated by ai-sdlc instruction hierarchy");
  });
  ...
  it("does not emit nested files for unaccepted or root hierarchy scopes", () => {
    ...
    expect(paths).not.toContain(".kiro/steering/src-core.md");
  });
});
