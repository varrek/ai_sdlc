...
describe("loop behavior eval synthetic traces", () => {
  it.each(
    LOOP_BEHAVIOR_SCENARIOS.map((scenario) => [scenario.id, scenario] as const),
  )("%s passes with valid trace", (scenarioId, scenario) => {
...
describe("loop role-choice eval with guidance bundles", () => {
  it.each(
    LOOP_ROLE_CHOICE_SCENARIOS.map((scenario) => [scenario.id, scenario] as const),
  )("%s selects correct role from guidance bundle", (scenarioId, scenario) => {
...
describe("loop evaluator handback eval with guidance bundles", () => {
  it.each(
    LOOP_HANDBACK_SCENARIOS.map((scenario) => [scenario.id, scenario] as const),
  )("%s selects correct handback role from guidance", (scenarioId, scenario) => {
    const root = copyFixture("python-rags");
    const artifacts = runSetup(root);
    const bundle = guidanceFromSetup(artifacts);
