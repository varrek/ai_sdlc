...
const NESTED_INSTRUCTION_FILENAMES = ["AGENTS.md", "CLAUDE.md"] as const;

/** True when instruction file contents were emitted by ai-sdlc (safe to overwrite). */
export function isGeneratedInstructionFile(contents: string): boolean {
  return contents.includes(GENERATED_INSTRUCTION_MARKER);
}

/**
 * True when a scope directory already carries hand-authored nested instruction
 * files that compile must not overwrite (R3 / host-native hierarchy plan).
 */
export function scopeHasUserOwnedInstructionFiles(repoRoot: string, scopePath: string): boolean {
  for (const fileName of NESTED_INSTRUCTION_FILENAMES) {
    const abs = join(repoRoot, scopePath, fileName);
    if (!existsSync(abs)) continue;
    if (!isGeneratedInstructionFile(readFileSync(abs, "utf8"))) return true;
  }
  return false;
}
